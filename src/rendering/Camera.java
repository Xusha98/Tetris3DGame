	...
	public void update(Window window) {
		...
		if (window.isKeyDown(GLFW.GLFW_KEY_W))
			this.addPosition((float) (Math.sin(Math.toRadians(rotation.getY()))) * -moveSpeed, 0,
					(float) (Math.cos(Math.toRadians(rotation.getY()))) * moveSpeed);
		if (window.isKeyDown(GLFW.GLFW_KEY_S))
			this.addPosition((float) (Math.sin(Math.toRadians(rotation.getY()))) * moveSpeed, 0,
					(float) (Math.cos(Math.toRadians(rotation.getY()))) * -moveSpeed);
		if (window.isKeyDown(GLFW.GLFW_KEY_D))
			this.addPosition((float) (Math.sin(Math.toRadians(rotation.getY() - 90))) * -moveSpeed, 0,
					(float) (Math.cos(Math.toRadians(rotation.getY() - 90))) * moveSpeed);
